...
public class Reflector {
  ...
  private void addDefaultConstructor(Class<?> clazz) {
    ...
    for (Constructor<?> constructor : consts) {
      ...
      if (constructor.getParameterTypes().length == 0) {
          //赋值给 defaultConstructor 旧版本会在赋值前检查Security权限
  ...
  // <3> 遍历所有方法
    ...
    // <4> 解决 getting 冲突方法 解决子类和父类覆盖时候同时存在的冲突
  ...
  private void resolveGetterConflicts(Map<String, List<Method>> conflictingGetters) {
    ...
    for (Entry<String, List<Method>> entry : conflictingGetters.entrySet()) {
      ...
      for (Method candidate : entry.getValue()) {
        // winner 为空，说明 candidate 为最匹配的方法 - winner初始化
        if (winner == null) {
          winner = candidate;
          continue;
        }
        ...
        if (candidateType.equals(winnerType)) {
          //不是boolean返回类型抛异常 因为返回类型相同get方法无法选择更优解
          // 应该在 getClassMethods 方法中只会拿签名相同的最近方法（类似就近原则）
          ...
          // 不符合选择子类
        } else if (candidateType.isAssignableFrom(winnerType)) { //candidate 是否是父类
          // OK getter type is descendant、
          // <1.1> 符合选择子类。因为子类可以修改放大返回值。例如，父类的一个方法的返回值为 List ，子类对该方法的返回值可以覆写为 ArrayList。
        } else if (winnerType.isAssignableFrom(candidateType)) { //winner 是否是父类
          winner = candidate;
  ...
  private void addMethodConflict(Map<String, List<Method>> conflictingMethods, String name, Method method) {
    //看map中是否有get方法name的key 没有就初始化new 一个ArrayList
  ...
  private void resolveSetterConflicts(Map<String, List<Method>> conflictingSetters) {
    ...
    for (String propName : conflictingSetters.keySet()) {
      ...
      for (Method setter : setters) {
        ...
        //第一个入参 和 getterType （返回类型） 相同，直接使用
  ...
  private Method pickBetterSetter(Method setter1, Method setter2, String property) {
    //如果setter1是空 则 setter2肯定是最匹配的
    if (setter1 == null) {
      return setter2;
    }
    Class<?> paramType1 = setter1.getParameterTypes()[0];
    Class<?> paramType2 = setter2.getParameterTypes()[0];
    //找两个方法哪个是子类 子类更符合需要的类需要-类似就近原则
    if (paramType1.isAssignableFrom(paramType2)) {
      return setter2;
    } else if (paramType2.isAssignableFrom(paramType1)) {
      return setter1;
    }
    //如果两个方法找不到谁是子类父类 则异常说明是重载不是重写
  ...
  private boolean isValidPropertyName(String name) {
    //全部满足 不能是 $ 开头 不能是 serialVersionUID 和 class 值
  ...
  private void addUniqueMethods(Map<String, Method> uniqueMethods, Method[] methods) {
    ...
    for (Method currentMethod : methods) {
      // 忽略 bridge 方法 查看是否是桥接方法，参见 https://www.zhihu.com/question/54895701/answer/141623158 文章
      if (!currentMethod.isBridge()) {
        // <3> 获得方法签名 格式为 返回类型#方法名:参数1,参数2...
