...
public class Configuration {
   ...
   * KeyGenerator 的映射
   *
   * KEY：在 {@link #mappedStatements} 的 KEY 的基础上，跟上 {@link org.apache.ibatis.executor.keygen.SelectKeyGenerator#SELECT_KEY_SUFFIX}
   */
  ...
  public void setLogImpl(Class<? extends Log> logImpl) {
    if (logImpl != null) {
      ...
      // 添加到 LogFactory 的 logConstructor
  ...
  protected void buildAllStatements() {
    parsePendingResultMaps();//incompleteCacheRefs 解析 防止深度嵌套出现的 参数异常
  ...
  private void parsePendingResultMaps() {
    if (incompleteResultMaps.isEmpty()) {//incompleteResultMaps 为空不解析
      return;
    }
    synchronized (incompleteResultMaps) {// 保证 incompleteCacheRefs 被解析完
      boolean resolved;
